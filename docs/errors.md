...
The server rejected a spend limit or auto-reload change you made from the [`/usage-credits` dialog](/en/costs#set-a-spend-limit-on-pro-and-max) or from the prompt that appears when you reach your spend limit.
...
* If turns keep failing behind the same proxy after updating, work through [Unable to connect to API](#unable-to-connect-to-api) and check the proxy setup in [Network configuration](/en/network-config)
...
Both forms name `/clear` instead of `/compact` when you have set [`DISABLE_COMPACT`](/en/env-vars).
...
The file you passed to [`--settings`](/en/cli-reference#cli-flags) is larger than 2 MiB, so `claude` exits with code 1 at startup instead of loading it. A settings file is a small JSON document, so a file this large usually means the path points at the wrong file. Before v2.1.214, Claude Code read the file with no size check, and a multi-gigabyte file or a device file such as `/dev/zero` grew memory without bound.
...
* Point `--settings` at a regular JSON settings file under 2 MiB. See [Settings](/en/settings) for the format.

### Workspace not trusted when starting Remote Control

You started [Remote Control](/en/remote-control) server mode with `claude remote-control` or its `claude rc` alias in a directory you haven't trusted. The command doesn't show the workspace trust dialog itself, so it exits with code 1 and names the fix:
...
* Run `claude` in the directory, accept the [workspace trust dialog](/en/permissions#project-allow-rules-and-workspace-trust), then run `claude remote-control` again
...
Claude Code couldn't read or process the saved transcript for the session you selected from the [`claude --resume` picker](/en/sessions#use-the-session-picker), so it ends the process rather than continue in a partially loaded state. The message includes the command to retry:
...
* To give the subagent every [subagent-available](/en/sub-agents#available-tools) tool the parent has, delete the `tools` field instead of listing tools
...
Commands that open an interactive dialog can't do so while no terminal is attached to a background session. `/install-github-app`, the `/mcp` settings list, and the authentication actions in the MCP server menu respond with a message, and the session appears under **Needs input** in [agent view](/en/agent-view) so you can find it, attach, and run the command again. While a terminal is attached, these commands work normally.
...
The warning names only the directories Claude Code searched, and it appears in the resumed conversation whether you wake a [background session](/en/agent-view), run `/resume` or `claude --resume`, or resume in [non-interactive mode](/en/headless), where it also goes to stderr. Sessions using `--input-format stream-json` don't show it, because the Agent SDK supplies agents after startup.
...
This warning comes from a [`/rewind`](/en/checkpointing) code restore. It reports paths the restore refused to touch; the restore completed for every other tracked file.
...
* If a skipped file is a link you created on purpose, such as a config file managed by a dotfile manager or a file hard-linked by tools like pnpm, restore its contents from [version control](/en/checkpointing#not-a-replacement-for-version-control) instead
